...
def attention_alignment_loss(
    ...
    gaze_probs: [B, N] (already normalized to sum=1)
        ...
        Layer selection:
            - if layer_start/layer_end are provided: attentions[layer_start:layer_end]
                (Python slicing; supports negative indices)
            - else: use the last `num_last_layers` layers (legacy behavior).
    """
    if len(attentions) == 0:
...
def main() -> None:
    ...
    def _forward_batch(b: Batch, update_idx: int) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        # Possibly disable gaze and use uniform target.
        ...
        if use_gaze:
            ...
            n = int(w.shape[1]) if w.shape[0] == bsz else int((w.shape[0] // bsz) * w.shape[1])
            gaze_target = torch.full((bsz, n), 1.0 / float(n), device=w.device, dtype=w.dtype)
